...
describe('job schemas — pay bounds and period', () => {
  it('rejects an upper bound below the lower bound', () => {
    expect(() =>
      mhdPayRangeSchema.parse({ jobId: 'job-1', payMin: 50000, payMax: 40000, payPeriod: 'ANNUAL' }),
  ...
  it('requires a period once either bound is set on a job form', () => {
    ...
    expect(mhdJobFormSchema.parse({ companyId: 'company-1', jobTitle: 'Cook' }).jobTitle).toBe('Cook');
  });

  it('accepts a valid O*NET-SOC code and rejects a malformed one', () => {
    expect(
      mhdJobFormSchema.parse({ companyId: 'company-1', jobTitle: 'Driver', onetSocCode: '53-3032.00' })
        .onetSocCode,
    ).toBe('53-3032.00');
    expect(() =>
      mhdJobFormSchema.parse({ companyId: 'company-1', jobTitle: 'Driver', onetSocCode: 'not-a-code' }),
...
describe('job schemas — publish gate and duplicates', () => {
  ...
  it('allows a null companyId on a competency — that is how a GLOBAL row is addressed', () => {
    expect(mhdCompetencySchema.parse({ companyId: null, competencyName: 'Leadership' }).companyId).toBeNull();
  });
});
...
describe('job schemas — pay-change audit metadata excludes the figures', () => {
  it('records pay_period only, with no pay_min or pay_max present at all', () => {
    ...
    expect(Object.keys(metadata).some((key) => /pay_(min|max)|amount|figure|salary/i.test(key))).toBe(
      false,
    );
  });
});
