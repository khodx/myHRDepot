...
vi.mock('@/features/esignature/Service', () => ({
  mhdEsignatureService: { createRequestFromGeneratedDocument: createRequestFromGeneratedDocumentMock },
}));
...
describe('mhdPerformanceService', () => {
  it('passes only the required list filter when optional filters are unset', async () => {
    ...
    await mhdPerformanceService.listReviews({
      companyId: 'company-1', personId: 'ALL', reviewerUserId: 'ALL', reviewType: 'ALL',
      status: 'ALL', searchTerm: '', dueFrom: '', dueTo: '',
    });

    expect(rpcMock).toHaveBeenCalledWith('mhd_list_performance_reviews', { p_company_id: 'company-1' });
  });
  ...
  it('uses the Activities contract and rejects unrelated activity types', async () => {
    ...
    expect(listActivitiesMock).toHaveBeenCalledWith(expect.objectContaining({
      companyId: 'company-1', personId: 'person-1', activityType: 'ONE_ON_ONE',
    }));

    await expect(mhdPerformanceService.createPersonActivity({
      companyId: 'company-1', activityType: 'MEETING', title: 'Not permitted',
    })).rejects.toThrow('The Performance module only creates ONE_ON_ONE or COACHING_SESSION activities.');
  ...
  it('reports each finalize ceremony step while using the subject as the only signer', async () => {
    const review = {
      ...
      reviewer_comments: null, employee_comments: null, review_activity_id: null, document_generation_id: null,
      document_generation_reference_id: null, document_generation_status: null, esignature_request_id: null,
      esignature_request_reference_id: null, esignature_request_status: null, acknowledged_at: null,
      waiver_reason: null, created_at: '2026-01-01T00:00:00Z', created_by: 'reviewer-1',
      updated_at: '2026-01-01T00:00:00Z', updated_by: 'reviewer-1',
    };
    ...
    returnsMock
      ...
      .mockResolvedValueOnce({ data: [{ id: 'gen-1', reference_id: 'DGEN-1', status: 'PENDING' }], error: null })
    ...
    createRequestFromGeneratedDocumentMock.mockResolvedValueOnce({ request: { id: 'signature-1' }, invitationErrors: [] });
    const steps: number[] = [];

    await mhdPerformanceService.finalizeReviewForSignature({
      reviewId: 'review-1', templateId: 'template-1', signer: { kind: 'internal', userId: 'subject-user-1' },
      pollIntervalMs: 0, onStep: (step) => steps.push(step),
    });

    expect(steps).toEqual([0, 1, 2, 3, 4, 5, 6, 7]);
    expect(createRequestFromGeneratedDocumentMock).toHaveBeenCalledWith(expect.objectContaining({
