...
export function MhdApprovalHistory({ entityType, entityId }: MhdApprovalHistoryProps) {
  ...
  if (isLoading) return <div className="text-sm text-muted-foreground">Loading approval history...</div>;
  ...
  return (
    <div className="space-y-4">
      ...
      {entries.length === 0 ? (
        <p className="text-sm text-muted-foreground">No approvals have been requested for this item.</p>
      ) : (
        entries.map(({ approval, comments }) => (
          <div key={approval.id} className="space-y-2 rounded border border-border p-3">
            <div className="flex items-center justify-between gap-3">
              <div>
                <Link to={`/approvals/${approval.id}`} className="font-medium text-accent hover:text-accent-hover">
