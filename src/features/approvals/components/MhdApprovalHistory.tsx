...
export function MhdApprovalHistory({ entityType, entityId }: MhdApprovalHistoryProps) {
  ...
  if (isLoading)
    return <div className="text-sm text-muted-foreground">Loading approval history...</div>;
    ...
    <div className="space-y-4">
      ...
      {entries.length === 0 ? (
        <p className="text-sm text-muted-foreground">
          No approvals have been requested for this item.
        </p>
      ) : (
        entries.map(({ approval, comments }) => (
          <div key={approval.id} className="space-y-2 rounded border border-border p-3">
            ...
            <p className="text-xs text-muted-foreground">
              Requested by {approval.requesterName || approval.requesterId} on{' '}
              {format(new Date(approval.createdAt), 'PPp')}
              {approval.resolvedAt
                ? ` · Resolved ${format(new Date(approval.resolvedAt), 'PPp')}`
                : ''}
            </p>
            {approval.reason ? <p className="text-sm text-foreground">{approval.reason}</p> : null}
            {comments.length > 0 ? (
              <ul className="space-y-1 border-t border-border pt-2">
                {comments.map((comment) => (
                  <li key={comment.id} className="text-xs text-muted-foreground">
                    <span className="font-medium">{comment.authorName || comment.userId}</span> (
                    {format(new Date(comment.createdAt), 'Pp')}): {comment.comment}
                  </li>
                ))}
              </ul>
            ) : null}
