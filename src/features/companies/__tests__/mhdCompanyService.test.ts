...
const { returnsMock, fromMock } = vi.hoisted(() => {
  ...
  const eqMock = vi.fn((): unknown => ({ order: orderMock, returns: returnsMock, eq: eqMock, ilike: ilikeMock }));
  const ilikeMock = vi.fn((): unknown => ({ order: orderMock, returns: returnsMock, eq: eqMock, ilike: ilikeMock }));
  const selectMock = vi.fn(() => ({ order: orderMock, eq: eqMock, ilike: ilikeMock, single: singleMock, returns: returnsMock }));
