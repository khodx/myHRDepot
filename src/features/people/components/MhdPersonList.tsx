...
export function MhdPersonList({ people, selectedPersonId, isLoading, onSelectPerson }: MhdPersonListProps) {
  if (isLoading) {
    return <div className="rounded-lg border border-slate-200 bg-white p-4 text-sm text-slate-500">Loading people...</div>;
  }

  if (people.length === 0) {
    return <div className="rounded-lg border border-dashed border-slate-300 bg-white p-4 text-sm text-slate-500">No people found.</div>;
  }
        ...
        {people.map((person) => {
          ...
          return (
            <li key={person.id}>
              <button
                type="button"
                className={`w-full px-4 py-3 text-left hover:bg-blue-50 ${selected ? 'bg-blue-50' : 'bg-white'}`}
