...
describe('mhdPersonService', () => {
  ...
  it('lists people using the approved directory RPC', async () => {
    ...
    expect(rpcMock).toHaveBeenCalledWith('mhd_list_people_directory', { p_company_id: undefined, p_search_term: 'maria' });
  ...
  it('throws a useful error when the RPC fails', async () => {
    ...
    await expect(mhdPersonService.listPeople({ companyId: 'ALL', searchTerm: '' })).rejects.toThrow('Unable to load people: permission denied');
  });
});
