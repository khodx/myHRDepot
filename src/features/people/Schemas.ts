...
export function mhdPersonDisplayName(values: Pick<MhdPersonFormValues, 'firstName' | 'lastName' | 'preferredName'>): string {
  const firstName = values.firstName.trim();
  const lastName = values.lastName.trim();
  const preferredName = values.preferredName.trim();
  return preferredName.length > 0 ? `${preferredName} ${lastName}`.trim() : `${firstName} ${lastName}`.trim();
}
