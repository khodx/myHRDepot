    ...
    <form className="space-y-4 rounded-xl border border-border bg-card p-6 shadow-sm" onSubmit={handleSubmit(onSubmit)}>
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">{assignment.itemName} assigned to {assignment.personDisplayName}</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div>
          ...
          <input className="w-full rounded-md border border-border px-3 py-2 text-sm" {...register('receiverTitle')} />
        </div>
        <div>
          <label className="mb-1 block text-sm font-medium text-foreground">Condition at Return</label>
          <input className="w-full rounded-md border border-border px-3 py-2 text-sm" {...register('returnConditionNotes')} />
        </div>
      </div>
      ...
      <div>
        ...
        <input className="w-full rounded-md border border-border px-3 py-2 text-sm" {...register('employeeReturnSignatureName')} />
